...
class MembersController extends ACMController {
  ...
  instantiateRoutes() {
    ...
    /**
     * @api {get} /api/v2/members?[attributeName=attributeValue]?limit Get Members
     * @apiDescription
    ...
    this.router.get('/', async (req, res) => {
      try {
        ...
        const query = Object.entries(req.query).filter(
          (queryParam) => queryParam[0] === 'limit',
        )

        let members

        // If we don't have any custom queries, just return all members
    ...
    /**
     * @api {get} /api/v2/members/:user_id Get Members
     * @apiDescription
    ...
    /**
     * @api {post} /api/v2/members/:user_id Create Member
     * @apiDescription
    ...
    /**
     * @api {delete} /api/v2/members/:user_id Delete Member
     * @apiDescription
